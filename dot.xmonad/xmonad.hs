...
myKeys conf = M.fromList $ [
    ...
    ((smodm, xK_Return), spawn "emacs"),                                -- launch emacs
    ...
    ((modm,  xK_j     ), refresh),                                      -- Resize viewed windows to the correct size
    ((modm,  xK_t     ), windows W.focusDown),                          -- Move focus to the next window
    ((modm,  xK_n     ), windows W.focusUp),                            -- Move focus to the previous window
    ((modm,  xK_m     ), windows W.focusMaster),                        -- Move focus to the master window
    ((smodm, xK_t     ), windows W.swapDown),                           -- Swap the focused window with the next window
    ((smodm, xK_n     ), windows W.swapUp),                             -- Swap the focused window with the previous window
    ((smodm, xK_m     ), windows W.swapMaster),                         -- Swap the focused window and the master window
    ((modm,  xK_h     ), sendMessage Shrink),                           -- Shrink the master area
    ((modm,  xK_s     ), sendMessage Expand),                           -- Expand the master area
    ((modm,  xK_k     ), withFocused $ windows . W.sink),               -- Push window back into tiling
    ((modm,  xK_comma ), sendMessage (IncMasterN 1)),                   -- Increment the number of windows in the master area
    ((modm,  xK_period), sendMessage (IncMasterN (-1))),                -- Deincrement the number of windows in the master area
    ((smodm, xK_q     ), io $ exitWith ExitSuccess),                    -- Quit xmonad
    ((modm,  xK_q     ), spawn "xmonad --recompile; xmonad --restart"), -- Restart xmonad
    ((modm,  xK_x     ), spawn "xrandr --output LVDS1 --off"),          -- Reset xrandr
    ((smodm, xK_x     ), spawn "xrandr --output LVDS1 --auto")]         -- Reset xrandr
    ++
